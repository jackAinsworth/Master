...
nohup python -u ISM_coons_op_3.py   --tfrecord_glob "/home/ainsworth/master/dataset/tfrecords_ruled/*.tfrecord"   --epochs 150   > "$LOG" 2>&1 &
...
def main(tfrecord_path_pattern: str, epochs: int):
    ...
    with strategy.scope():
        ...
        model.compile(
           optimizer=tf.keras.optimizers.AdamW(learning_rate=1e-4, weight_decay=1e-4), #2e-4
            loss=total_loss,
            #loss="mse",
            metrics=['mse']
        )
