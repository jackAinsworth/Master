...
Example:
  python train_pointnet_surface_torch.py \
      ...
      LOG=logs/pointnet_surface_fa_$(date +%Y%m%d_%H%M).log   # e.g. pointnet_surface_20250618_1423.log
CUDA_VISIBLE_DEVICES=1 \

nohup python -u train_pointnet_surface_torch_fa_1.py \
      --tfrecord_glob "./tfrecords_pointcloud/data_*.tfrecord" \
      --epochs 100 \
      > "$LOG" 2>&1 &
...
class TotalLoss(nn.Module):
    ...
    def __init__(self, w_lap: float = 0.05):
        super().__init__()
        self.w_lap = w_lap
...
class PointNet2SurfaceRegressor(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp_head = nn.Sequential(
            nn.Linear(self.global_in_ch, 1024),
            nn.ReLU(inplace=True),
            nn.GroupNorm(1, 1024),  # replaces BatchNorm1d(1024)
            nn.Dropout(0.10),

            nn.Linear(1024, 512),
            nn.ReLU(inplace=True),
            nn.GroupNorm(1, 512),  # replaces BatchNorm1d(512)
            nn.Dropout(0.10),
        )
...
def main(tfrecord_glob: str, epochs: int):
    ...
    #train_steps = NUM_EXAMPLES * 0.8 // BATCH_SIZE  # 4 687
    #val_steps = NUM_EXAMPLES * 0.2 // BATCH_SIZE
    best_val = float('inf')

    # ── steps calculation (ints only)
    total_train_examples = int(NUM_EXAMPLES * 0.8)
    total_val_examples = NUM_EXAMPLES - total_train_examples
