...
python incept_conv1d.py \
  --tfrecord_glob "./tfrecords_pointcloud/data_*.tfrecord" \
  --epochs 20


nohup python incept_conv1d.py \
  --tfrecord_glob "./tfrecords_pointcloud/data_*.tfrecord" \
  --epochs 20 \
  > logs/train_incept_conv1d.log 2>&1 &
"""
...
def inception_module_1d(x, n):
    """
    Inception module as described:
    Path 1: Conv1D(n, kernel_size=1)
    Path 2: Conv1D(n, kernel_size=1) → Conv1D(n, kernel_size=3)
    Path 3: Conv1D(n, kernel_size=1) → Conv1D(n, kernel_size=5)
    Path 4: MaxPooling1D(kernel_size=3) → Conv1D(n, kernel_size=1)
    """
    # Path 1
    path1 = layers.Conv1D(n, kernel_size=1, padding='same', activation='relu')(x)

    # Path 2
    path2 = layers.Conv1D(n, kernel_size=1, padding='same', activation='relu')(x)
    path2 = layers.Conv1D(n, kernel_size=3, padding='same', activation='relu')(path2)

    # Path 3
    path3 = layers.Conv1D(n, kernel_size=1, padding='same', activation='relu')(x)
    path3 = layers.Conv1D(n, kernel_size=5, padding='same', activation='relu')(path3)

    # Path 4
    path4 = layers.MaxPooling1D(pool_size=3, strides=1, padding='same')(x)
    path4 = layers.Conv1D(n, kernel_size=1, padding='same', activation='relu')(path4)

    # Concatenate all paths
    output = layers.Concatenate(axis=-1)([path1, path2, path3, path4])
    return output
