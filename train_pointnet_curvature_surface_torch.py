...
Example:
  python train_pointnet_curvature_surface_torch.py \
      --tfrecord_glob "./tfrecords_pointcloud/data_*.tfrecord" \
      --epochs 100


      LOG=logs/pointnet_curvature_surface_$(date +%Y%m%d_%H%M).log   # e.g. pointnet_surface_20250618_1423.log
CUDA_VISIBLE_DEVICES=1 \

nohup python -u train_pointnet_curvature_surface_torch.py \
      --tfrecord_glob "./tfrecords_pointcloud/data_*.tfrecord" \
      --epochs 100 \
      > "$LOG" 2>&1 &
...
MAX_CTRLS_U  = 10
MAX_CTRLS_V  = 10
BATCH_SIZE   =  32 #256 #128            # tune to your GPU memory
...
def estimate_curvature(xyz: torch.Tensor, k: int = 16) -> torch.Tensor:
    """
    xyz: (N,3) point cloud
    k:  number of neighbors for local covariance

    returns curvature: (N,) where higher means more curved
    """
    N, _ = xyz.shape

    # 1) For each point, find its kNN (Euclidean)
    #    We'll just brute-force here; for large N use faiss or a spatial tree.
    dists = torch.cdist(xyz, xyz, p=2)            # (N, N)
    knn_idx = dists.argsort(dim=1)[:, 1 : k+1]    # skip self at idx 0

    # 2) Gather neighbor coordinates
    neighbors = xyz[knn_idx]                      # (N, k, 3)
    diffs     = neighbors - xyz.unsqueeze(1)      # (N, k, 3)

    # 3) Compute local covariance for each point:
    #    cov_i = (1/k) * diffs_i^T @ diffs_i
    #    We'll compute the 3×3 covariance per point in a batched way
    cov = diffs.transpose(1,2) @ diffs            # (N, 3, 3)
    cov = cov / k

    # 4) Eigen‐decompose each cov_i → eigenvalues λ0 ≤ λ1 ≤ λ2
    #    curvature ≈ λ0 / (λ0 + λ1 + λ2)
    #    torch.linalg.eigh returns them in ascending order
    vals, _ = torch.linalg.eigh(cov)              # vals: (N, 3)
    curvature = vals[:, 0] / (vals.sum(dim=1) + 1e-6)
    return curvature.clamp(min=0)


def curvature_weighted_fps(xyz: torch.Tensor,
                           curvature: torch.Tensor,
                           npoint: int,
                           alpha: float = 1.0) -> torch.Tensor:
    """
    xyz:        (N,3)  point cloud
    curvature: (N,)    per-point curvature
    npoint:     int    how many samples to pick
    alpha:      float  curvature exponent
    → returns idx: (npoint,) indices of selected centroids
    """
    N, _ = xyz.shape
    device = xyz.device

    # Initialize: pick the highest-curvature point as first centroid
    first = torch.argmax(curvature)
    centroids = [first.item()]

    # Track for each point its min-distance to any picked centroid
    dist2 = torch.full((N,), float('inf'), device=device)

    for _ in range(1, npoint):
        # Update min-distances to the newest centroid
        last = centroids[-1]
        d = torch.sum((xyz - xyz[last])**2, dim=1)   # (N,)
        dist2 = torch.minimum(dist2, d)

        # Compute curvature-weighted score
        score = (curvature ** alpha) * torch.sqrt(dist2)

        # Mask out already-picked ones
        score[centroids] = -1.0

        # Pick the next argmax
        next_idx = torch.argmax(score)
        centroids.append(next_idx.item())

    return torch.tensor(centroids, dtype=torch.long, device=device)
...
class PointNetSetAbstractionKNN(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, points: torch.Tensor | None):
        ...
        # Precompute curvature once per input cloud
        curv = estimate_curvature(xyz.permute(0, 2, 1).reshape(-1, 3), k=16)
        curv = curv.view(B, N)  # (B,N)

        # In your forward():
        fps_idx = []
        for b in range(B):
            idx_b = curvature_weighted_fps(
                xyz[b].transpose(0, 1),  # (N,3)
                curv[b],  # (N,)
                self.npoint,
                alpha=1.0
            )
            fps_idx.append(idx_b)
        fps_idx = torch.stack(fps_idx, dim=0)  # (B, npoint)
        new_xyz = index_points(xyz.permute(0, 2, 1), fps_idx)
...
# Updated PointNet2SurfaceRegressor with 4 KNN SA blocks and expanded MLP head:
class PointNet2SurfaceRegressor(nn.Module):
    """Scaled-up PointNet++ surface regressor with kNN SA and optional pooling"""
    def __init__(self,
        ...
        # 1) Embed raw XYZ → feature
        self.input_mlp = nn.Sequential(
            nn.Conv1d(3, 64, 1, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
        )
        # 2) Optional pooling of coords+features
        #self.input_pool_max = nn.MaxPool1d(2, 2)
        #self.input_pool_avg = nn.AvgPool1d(2, 2)
        # 3) Four SA-KNN blocks
        self.sa1 = PointNetSetAbstractionKNN(num_points // 2, 32, 64, [64, 64, 128])
        self.sa2 = PointNetSetAbstractionKNN(num_points // 4, 32, 128, [128, 128, 256])
        self.sa3 = PointNetSetAbstractionKNN(num_points // 16, 32, 256, [256, 256, 512])
        self.sa4 = PointNetSetAbstractionKNN(num_points // 32, 32, 512, [512, 512, 1024])
        # 4) Dense head
        self.mlp_head = nn.Sequential(
            nn.Linear(1024, 1024), nn.ReLU(), nn.BatchNorm1d(1024), nn.Dropout(0.1),
            nn.Linear(1024, 512),  nn.ReLU(), nn.BatchNorm1d(512),  nn.Dropout(0.1),
        )
        self.out_linear = nn.Linear(512, output_ctrlpts_u * output_ctrlpts_v * 3)

    def forward(self, xyz_b_n3: torch.Tensor):
        # xyz: (B,N,3)
        xyz = xyz_b_n3.permute(0, 2, 1)  # (B,3,N)
        # Embed coords
        feats = self.input_mlp(xyz)      # (B,64,N)
        # Pool both coords and feats
        #feats = self.input_pool_max(feats)   # (B,64,N/2)
        #xyz   = self.input_pool_max(xyz)     # (B,3, N/2)
        # SA blocks
        ...
        # Global max pool
        global_feat = torch.max(l4_pts, dim=2)[0]  # (B,1024)
        # Dense head → ctrl
        x = self.mlp_head(global_feat)            # (B,512)
        ctrl = self.out_linear(x).view(
            -1, self.output_u, self.output_v, 3
        )                                         # (B,U,V,3)
        return ctrl
